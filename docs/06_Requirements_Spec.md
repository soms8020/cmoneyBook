...
| ID | 요구사항 | 우선순위 | 비고 |
|----|----------|----------|------|
| FR-A01 | 이메일과 비밀번호를 입력하여 새로운 계정을 생성할 수 있어야 한다. | 필수 | 비밀번호 암호화 |
| FR-A02 | 이메일과 비밀번호로 로그인할 수 있어야 한다. | 필수 | JWT 반환 |
| FR-A03 | 로그인한 사용자만 서비스 기능(데이터 조회/수정)을 이용할 수 있어야 한다. | 필수 | 401/403 에러 |
| FR-A04 | 사용자는 로그아웃할 수 있어야 한다. | 필수 | 클라이언트 토큰 삭제 |
| FR-A05 | 각 사용자는 본인이 등록한 데이터만 볼 수 있어야 한다. | 필수 | 데이터 고립성 |
...
### UC-00: 사용자 회원가입 및 로그인
1. "회원가입" 페이지 접근
2. 이메일, 비밀번호, 이름 입력 후 "가입 완료" 버튼 클릭
3. 성공 시 로그인 페이지로 이동
4. 이메일, 비밀번호 입력 후 "로그인" 버튼 클릭
5. 성공 시 대시보드로 이동 (JWT 토큰 발급)
